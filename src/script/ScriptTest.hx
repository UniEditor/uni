...
class ScriptTest {
	...
	static function test(x, v:Dynamic, ?vars : Dynamic) {
		...
		if ( v != ret )
			trace( ret + " returned while " + v + " expected");
		else {
			trace("SUCCESS: " +x +" = "+ ret);
		}
	}
	
	public static function doScript(script:String, times:Int) {
		...
		trace("run in :" + (endTime - startTime) / 1000 + "s");
	}
	
	public static function benchMark1() {
		var script = "var res = 0;\r var i = 0;  \rwhile(i < 10000*0000){i=i+1;}";
		doScript(script , 1);
	}
	
	public static function benchMark2() {
		var script = "var res = 0;\r var i = 0;  \rwhile(i < 10000*0000){i=i+1; res = res*2;}";
		doScript(script , 100);
	}
	...
	public static function main() {
		...
		test("for( x in [4,5,6] ) trace(x); 5", 5);
		test("var x = 1; while(x < 5){5; x++;} 5", 5);
